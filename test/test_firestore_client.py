...
def test_firestore_client_init_sets_db():
    with patch("firestore_client.firestore.Client") as MockClient:
        mock_instance = MockClient.return_value
        client = FirestoreClient("testcoll")
        assert client.db == mock_instance

def test_read_document_calls_collection_document():
    with patch("firestore_client.firestore.Client") as MockClient:
...
def test_create_document_calls_collection_add():
    with patch("firestore_client.firestore.Client") as MockClient:
        ...
        client = FirestoreClient("testcoll")
        result = client.create_document("testcoll", {"foo": "bar"})
        assert result == {"id": "123", "foo": "bar"}
        mock_db.collection.assert_called_with("testcoll")
        mock_collection.document.assert_called()
        mock_collection.document.return_value.set.assert_called_with({"foo": "bar"})
